...
const EMPTY = { name: '', phone: '', password: '', role: 'viewer' }

export default function UsuariosPanel({ user }: { user: User }) {
  const [usuarios, setUsuarios] = useState<Viewer[]>([])
...
async function fetchUsuarios() {
  ...
  function openEdit(v: Viewer) {
    setEditing(v)
    setForm({ name: v.name, phone: v.phone, password: '', role: v.role })
    setError('')
    setShowPassword(false)
    setAdminConfirm(false)
    setModal(true)
  ...
  async function handleSave() {
    ...
    if (editing) {
      const updates: any = { name: form.name, phone: form.phone, role: form.role }
      ...
      const { error: err } = await supabase
        .from('users')
        .insert({ name: form.name, phone: form.phone, password: form.password, role: form.role })
  ...
  const filtered = usuarios.filter(v =>
  ...
  return (
    <>
      <style>{`
        ...
        .uv-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 1.2rem; }
        .uv-stat { background: #fff; border: 1.5px solid #E4E8F0; border-radius: 0; padding: 1rem; }
        .uv-stat.gold { border-color: rgba(245,166,35,0.3); background: rgba(245,166,35,0.06); }
        .uv-stat.blue { border-color: rgba(43,91,168,0.2); background: rgba(43,91,168,0.04); }
        .uv-stat-label { font-size: 11px; color: #8A9CC0; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 6px; }
        .uv-stat.gold .uv-stat-label { color: #B8760A; }
        .uv-stat.blue .uv-stat-label { color: #1A3A6B; }
        .uv-stat-num { font-family: 'Sora', sans-serif; font-size: 24px; font-weight: 700; color: #0D1F45; }
        .uv-stat.gold .uv-stat-num { color: #B8760A; }
        .uv-stat.blue .uv-stat-num { color: #1A3A6B; }
        ...
        .uv-thead { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 80px; padding: 10px 16px; background: #F7F9FD; border-bottom: 1.5px solid #E4E8F0; }
        .uv-th { font-size: 11px; font-weight: 700; color: #8A9CC0; text-transform: uppercase; letter-spacing: 0.05em; }
        .uv-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 80px; padding: 12px 16px; border-bottom: 1px solid #F0F3FA; align-items: center; transition: background 0.15s; }
        ...
        .b-admin { display: inline-block; background: rgba(245,166,35,0.12); color: #B8760A; font-size: 11px; font-weight: 700; padding: 2px 9px; border-radius: 20px; }
        ...
        .uv-modal { background: #fff; border-radius: 16px; width: 100%; max-width: 440px; overflow: hidden; }
        ...
        .role-btn { padding: 10px; border: 1.5px solid #E4E8F0; border-radius: 9px; background: #F7F9FD; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; color: #8A9CC0; transition: all 0.18s; text-align: center; }
        .role-btn.active-viewer { border-color: #1A3A6B; background: #EBF0FC; color: #1A3A6B; }
        .role-btn.active-admin { border-color: #F5A623; background: rgba(245,166,35,0.1); color: #B8760A; }
        ...
        .confirm-modal { background: #fff; border-radius: 16px; width: 100%; max-width: 400px; padding: 1.5rem; text-align: center; }
        ...
        .btn-warning { padding: 9px 20px; border: none; border-radius: 9px; background: #F5A623; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 700; color: #0D1F45; cursor: pointer; }

        .admin-warning { background: rgba(245,166,35,0.08); border: 1.5px solid rgba(245,166,35,0.3); border-radius: 10px; padding: 1rem; }
        .admin-warning-title { font-size: 13px; font-weight: 700; color: #B8760A; margin-bottom: 4px; }
        .admin-warning-sub { font-size: 12px; color: #92660A; margin-bottom: 0.8rem; line-height: 1.6; }
        .admin-warning-btns { display: flex; gap: 8px; }
        .btn-confirm-admin { padding: 7px 14px; border: none; border-radius: 8px; background: #F5A623; color: #0D1F45; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 700; cursor: pointer; }
        .btn-cancel-admin { padding: 7px 14px; border: 1.5px solid #E4E8F0; border-radius: 8px; background: #fff; color: #4A6080; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; }

        @media (max-width: 768px) {
          .uv-stats { grid-template-columns: 1fr 1fr; }
      ...
      <div className="uv-wrap">
        <div className="uv-topbar">
          <div>
            ...
            <div className="uv-topbar-sub">{usuarios.length} usuarios registrados</div>
          </div>
            ...
            <div className="uv-table">
              ...
              {filtered.length === 0 ? (
                ...
                <div key={v.id} className="uv-row">
                  ...
                  <div>
                    {v.role === 'admin'
                      ? <span className="b-admin">Administrador</span>
        ...
        <div className="overlay" onClick={() => setModal(false)}>
          <div className="uv-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="uv-modal-body">
              ...
              <div>
                <label className="field-label">
                  Contraseña{' '}
                  {editing && <span style={{ color: '#8A9CC0', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(vacío = no cambiar)</span>}
                </label>
                ...
                <div className="field-desc">El usuario usa su teléfono y contraseña para ingresar</div>
              </div>
              ...
              <div>
                ...
                <div className="role-grid">
                  <button
                    className={`role-btn ${form.role === 'viewer' ? 'active-viewer' : ''}`}
                    onClick={() => handleRoleChange('viewer')}
                  >
                    👁 Visualizador
                  </button>
                  <button
                    className={`role-btn ${form.role === 'admin' ? 'active-admin' : ''}`}
              ...
              {adminConfirm && (
                <div className="admin-warning">
                  <div className="admin-warning-title">⚠️ ¿Crear administrador?</div>
                  <div className="admin-warning-sub">
                    Darle rol de administrador a este usuario le permitirá <strong>crear, editar y eliminar</strong> cualquier registro del sistema. Esta acción puede comprometer la integridad de los datos.
                  </div>
                  <div className="admin-warning-btns">
                    <button className="btn-confirm-admin" onClick={confirmarAdmin}>
                      Sí, asignar como admin
                    </button>
                    <button className="btn-cancel-admin" onClick={() => setAdminConfirm(false)}>
                      Cancelar
                    </button>
                  </div>
                </div>
            ...
            <div className="uv-modal-foot">
              ...
              <button className="btn-save" onClick={handleSave} disabled={saving || adminConfirm}>
